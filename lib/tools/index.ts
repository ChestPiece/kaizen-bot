...
const getMyLeads = tool({
  ...
  execute: async (args, { context }) => {
    const { agentId } = context as { agentId: string }
...
const searchLeads = tool({
  description:
    'Use when searching for a specific lead by name or phone number, or filtering leads across the team by status, property type, or area. Use this when the agent mentions a specific person or wants to find leads matching criteria.',
  parameters: z.object({
    query: z.string().optional().describe('Name or phone number to search for (case-insensitive partial match).'),
    status: z
      .enum(['new', 'contacted', 'qualified', 'negotiating', 'closed_won', 'closed_lost'])
      .optional()
      .describe('Filter by lead status.'),
    property_type: z.enum(['residential', 'commercial']).optional().describe('Filter by property type.'),
    area: z.string().optional().describe('Filter by preferred area (e.g. "Downtown", "JBR").'),
    assigned_to: z.string().uuid().optional().describe('Filter by agent UUID. Use only if explicitly requested.'),
  }),
  execute: async (args, { context }) => {
    const { agentId } = context as { agentId: string }
...
const updateLeadStatus = tool({
  ...
  parameters: z.object({
    ...
    reason: z
      .string()
      .optional()
      .describe('Optional reason for the status change. Will be saved as a note on the lead.'),
  }),
  execute: async (args, { context }) => {
    const { agentId } = context as { agentId: string }
...
const createLead = tool({
  ...
  parameters: z.object({
    full_name: z.string().min(1).describe('Full name of the lead.'),
    phone: z.string().min(1).describe('Phone number including country code.'),
    email: z.string().email().optional().describe('Email address (optional).'),
    nationality: z.string().min(1).describe('Nationality of the lead (e.g. "UAE", "British", "Indian").'),
    property_type: z.enum(['residential', 'commercial']).describe('Type of property the lead is interested in.'),
    intent: z.enum(['buy', 'rent', 'invest']).describe("Lead's intent — buying, renting, or investing."),
    budget_aed: z.number().positive().describe('Budget in AED (e.g. 3000000 for 3M AED).'),
    preferred_areas: z
      .array(z.string())
      .optional()
    ...
    source: z
      .string()
      .optional()
      .describe('How the lead was acquired (e.g. "referral", "website", "Instagram").'),
  }),
  execute: async (args, { context }) => {
    const { agentId } = context as { agentId: string }
